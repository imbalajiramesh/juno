...
# Fixed script to create Stripe products and prices for Juno credit packages
# Usage: ./create-stripe-products-fixed.sh [test|live]
...
create_package() {
    ...
    # Create product with correct metadata syntax
    product_output=$(stripe products create $LIVE_FLAG \
        --name "Juno Credits - $name" \
        --description "$description - $credits communication credits for voice calls, SMS, emails, and phone numbers" \
        -d "metadata[package_name]=$name" \
        -d "metadata[credits]=$credits" \
        -d "metadata[credits_per_dollar]=$credits_per_dollar" \
        -d "metadata[is_popular]=$is_popular" \
        -d "metadata[juno_package]=true" 2>&1)
    ...
    if [ -n "$product_id" ]; then
        ...
        price_output=$(stripe prices create $LIVE_FLAG \
            --product "$product_id" \
            --unit-amount "$price_cents" \
            --currency usd \
            -d "metadata[package_name]=$name" \
            -d "metadata[credits]=$credits" 2>&1)
