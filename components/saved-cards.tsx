...
export default function SavedCards({ onCardAdded }: SavedCardsProps) {
  ...
  // Check if Stripe is configured - do this in useEffect to ensure client-side execution
  const [isStripeConfigured, setIsStripeConfigured] = useState(false);

  // Check Stripe configuration and initialize promise when component mounts
  useEffect(() => {
    const publishableKey = process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY;
    console.log('Stripe publishable key check:', publishableKey ? 'Present' : 'Missing');
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-2 text-muted-foreground">Loading saved cards...</span>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardContent className="space-y-4">
        ...
        <Dialog open={showAddCard} onOpenChange={setShowAddCard}>
          <DialogContent className="sm:max-w-md">
            ...
            {setupClientSecret && isStripeConfigured && stripePromise && (
              <Elements
