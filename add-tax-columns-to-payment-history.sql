-- Add tax-related columns to payment_history table
...
-- Add a check constraint to ensure total = subtotal + tax
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'chk_payment_total_equals_subtotal_plus_tax' 
        AND table_name = 'payment_history'
    ) THEN
        ALTER TABLE payment_history 
        ADD CONSTRAINT chk_payment_total_equals_subtotal_plus_tax 
        CHECK (amount_usd_cents = subtotal_usd_cents + tax_amount_usd_cents);
    END IF;
END $$;
