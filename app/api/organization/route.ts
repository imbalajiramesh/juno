...
export const DELETE = withTenant(async (tenantId: string, request: NextRequest) => {
  ...
  try {
    ...
    await Promise.all([
      ...
      supabase.from('juno_call_logs').delete().eq('tenant_id', tenantId),
      supabase.from('juno_email_logs').delete().eq('tenant_id', tenantId),
      supabase.from('juno_sms_logs').delete().eq('tenant_id', tenantId),
